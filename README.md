...
**サンプルは全件が「本体 ; その逆」で初期状態に戻ることを毎回検査している**
（`test/example_test.ml`）。可逆プログラムなら必ず全変数が 0 / nil に戻るという
定義そのもので、「最後まで走る」よりずっと強い。2026-08-03 にこの検査を入れた
ところ 30 本が引っかかり（古い値を捨てる `d -= d; d += new` と、表明が恒真になる
`delocal int x = x`）、すべて書き直した。定石は `coq/README.md` の「往復テスト」節。
...
| `example_test` | `example/*.rplpp` 全件の回帰（パース／`invert(invert p)=p`／`parse(pretty p)=p`／実行／**往復**）＋代表例の値 |
...
```
cd python && python3 -m pytest tests -q     # 219 件（差分163 / 診断22 / エラー経路34）
```
...
同じやり方で 2026-08-03 に**代入と局所ブロックの副条件の検査漏れ**も見つかった——
`x += x` や `a[0] += a[0]` のように**書き込みが自分の右辺（または添字）を変える文**、
および `delocal int x = x` のように**表明が恒真になる局所ブロック**は逆向きに戻せない
ので意味論には導出が無いが、実装は通していた。どちらも修正済み。
ブロックにしない **`new` / `delete`** も対応済み（オブジェクトブロックの前半・後半を
切り出したもの。反転で互いに移る）。
**値渡しの引数**も対応済み——値渡しの仮引数は局所ブロックで包まれ、その出口の表明が
そのまま「値引数を書き換えてはならない」という可逆性の条件になる（`bind_args`）。
**配列の長さと範囲検査**も入れた——長さは環境の `cells : cid -> nat` から引くので、
状態の形を変えずに済む。書き込みと要素交換は意味論の側で範囲を検査する。**式の中の読み出し**は `exec` では
検査しない（範囲外の読み出しは可逆性を壊さない）が、**実行可能インタプリタ `run`
の側で実装と同じ検査を入れてある**（`run_sound` は片側の含意なので定理は保たれる。
詳細と対応関係は `coq/README.md`）。負の添字とゼロ除算も同じ扱いで、`run` の側だけが
拒否する。
**二項演算は `lib/syntax.ml` の 16 個すべて**を形式化してある。除算と剰余は OCaml と
同じく 0 方向へ切り捨てる（`Z.quot` / `Z.rem`。Coq 既定の `Z.div` / `Z.modulo` は
下方向に丸めるので負の被除数で食い違う）。
