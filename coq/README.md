...
| **`exec_steps`** | **大ステップとの対応**：`exec s a b` なら `(•s, a) →* (s•, b')` で `b == b'` |
...
対象は制御構造の核（skip・可逆代入・swap・並び・条件分岐・ループ）で、これを
述語 `core` で表している。局所ブロック・オブジェクト・呼出しへの拡張は同じ形の
規則を足す作業。

**大ステップとの対応**は `exec_steps` で片方向を証明した：`core s` なら
`exec G s a b` から `(•s, a) →* (s•, b')`（`b == b'`）が従う。証明は `exec` と
`loopx` の相互帰納で、ループの場合は「s1 を走る → 出口表明で判定 → s2 を走る →
入口表明で判定 → 次の周回」という小ステップの並びを組み立てる。系として
`steps_inj_from_exec`（同じ終状態に至る 2 つの実行の初期状態は一致する）も得られる。

逆方向（`(•s, a) →* (s•, b)` なら `exec G s a b`）は未証明。多ステップ列を
文脈ごとに分解する補題（いわゆる inversion lemma for →*）が要る。
